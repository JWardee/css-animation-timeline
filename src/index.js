module.exports = class CssAnimationTimeline {
  ...
  _playKeyframes(keyframes) {
    ...
    const frames = keyframes.map(
      (keyframe, keyframeIndex) => () => this._playKeyframe(keyframeIndex),
    );

    return frames.reduce(
      (previousFrame, nextFrame) => previousFrame.then(nextFrame),
      Promise.resolve(),
    ).then(() => {
      this._isPlaying = false;
    });
  ...
  _playKeyframe(keyframeIndex) {
    ...
    const keyframe = this._getKeyframe(keyframeIndex);
  ...
  _unwatchElement(el, keyframeIndex) {
    ...
    if (keyframe.staggerEach !== 0) {
      this._animationEndingEvents.forEach(
        eventName => el.removeEventListener(eventName, keyframe.finishCallback),
      );
    }
  }
  ...
  _watchElement(el, keyframeIndex, eventFunctionTrigger) {
    ...
    if (keyframe.staggerEach === 0) {
      this._animationEndingEvents.forEach(
        eventName => el.addEventListener(eventName, eventFunctionTrigger),
      );
    } else {
